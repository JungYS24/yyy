...
public class EnemyCTR : MonoBehaviour
{
    ...
    void Update()
    {
        if(player != null)
        {
            ...
            Quaternion targetRetation = Quaternion.LookRotation(targetDirection);
            EnemyPivot.transform.rotation =
                Quaternion.Lerp(EnemyPivot.transform.rotation, targetRetation, rotationSpeed * Time.deltaTime);
